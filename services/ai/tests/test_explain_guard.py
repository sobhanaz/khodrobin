...
def test_an_explanation_using_only_given_numbers_passes():
    ...
    ok, bad = explain.check(text, FACTS)
    assert ok, bad
...
def test_natural_unit_scaling_is_accepted():
    ...
    ok, _ = explain.check("میانه حدود ۲۱۱۴ میلیون تومان است.", FACTS)
    assert ok
    ok, _ = explain.check("ارزان‌ترین گزینه ۱۶۶۰ میلیون تومان است.", FACTS)
    assert ok


def test_small_numbers_are_ignored():
    # «۳ منبع», «۱۰ آگهی» are prose, not claims about the car.
    ok, _ = explain.check("این خودرو ۱۰ آگهی از ۳ منبع دارد.", FACTS)
    assert ok
...
def test_a_model_name_that_is_a_number_is_not_a_claim():
    ...
    ok, bad = explain.check("پژو ۲۰۷ گزینه‌ی خوبی است.", FACTS)
    assert ok, bad
    # But a number that is neither the name nor a given fact still fails.
    ok, bad = explain.check("پژو ۲۰۷ با ۳۳۳٬۳۳۳٬۳۳۳ تومان.", FACTS)
    assert not ok
...
def test_a_difference_between_two_given_numbers_is_a_derived_fact():
    ...
    ok, bad = explain.check("۱۸۰۰۰ کیلومتر کمتر از گزینه‌ی بعدی کار کرده است.", FACTS)
    assert ok, bad
    # A price gap between two given offers is allowed for the same reason.
    ok, bad = explain.check("۲۶۰٬۰۰۰٬۰۰۰ تومان ارزان‌تر است.", FACTS)
    assert ok, bad
...
def test_arbitrary_arithmetic_is_still_rejected():
    ...
    ok, bad = explain.check("مجموع این دو ۳٬۵۸۰٬۰۰۰٬۰۰۰ تومان می‌شود.", FACTS)
    assert not ok
