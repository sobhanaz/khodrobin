  ...
  <div>
    ...
    <div class="overflow-x-auto rounded-2xl border border-white/[.07] bg-surface">
      <table class="w-full min-w-[680px] text-[.84rem]">
        <thead>
          <tr class="border-b border-white/[.07] text-right text-[.74rem] text-ink-3">
            <th v-for="c in cols" :key="c" class="whitespace-nowrap px-4 py-3 font-normal">{{ c }}</th>
          </tr>
        </thead>
        <tbody>
          <tr v-if="loading">
            <td :colspan="cols.length" class="px-4 py-8 text-center text-ink-3">در حال بارگذاری…</td>
          </tr>
          <tr v-else-if="failed">
            <td :colspan="cols.length" class="px-4 py-8 text-center text-accent">بارگذاری نشد.</td>
          </tr>
          <tr v-else-if="!rows.length">
            <td :colspan="cols.length" class="px-4 py-8 text-center text-ink-3">چیزی برای نمایش نیست.</td>
          </tr>
    ...
    <div class="mt-3 flex items-center justify-between gap-3 text-[.78rem] text-ink-3">
      <span dir="rtl">
        <template v-if="total">
      ...
      <div class="flex gap-2">
        <button
          ...
          class="rounded-full border border-white/[.12] px-3 py-1.5 transition hover:text-ink disabled:opacity-35 disabled:hover:text-ink-3"
          @click="emit('page', -1)"
        >قبلی</button>
        <button
